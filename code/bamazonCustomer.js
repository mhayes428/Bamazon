    ...
    con.query(`SELECT * FROM products`, (err, res) => {
        ...
        inquirer.prompt([{
            ...
            con.query(`SELECT * FROM products WHERE item_id=${id}`, (err, res) => {
                ...
                let amountWanted = answers.quantity; // Put however many the user wants into variable
                let inStock = res[0].stock_quantity; // how much in stock currently
                let item = res[0].product_name; // item the user wants
                let price = res[0].price; // price item user wants
                ...
                switch (amountWanted <= inStock) {
                    case true:
                        ...
                        price = parseInt(price * amountWanted); 
...
const updateStore = (newTotal, id, newSales) => { 
